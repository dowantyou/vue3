...
<template>
  <div class="footer" ref="footerContainer" @mousedown="handleMouseDown">
    <!-- 作品版权归作者所有，如果侵犯了您的版权，请联系我们，本站将在3个工作日内删除。<br /> -->
    © 1968 Company Co. All rights reserved.
    <!-- <br />Copyright © 2024 light-word.com All Rights Reserved. -->
    <transition name="fade">
      <div v-show="isFooterVisible" class="px-4 divide-y dark:bg-gray-100 dark:text-gray-800">
        <!-- footer 内容 -->
...
const handleMouseMove = (event) => {
  if (isLongPress) {
    // 检测是否向上移动
    if (event.clientY < window.innerHeight - 20) { // 例如，向上移动超过 20px 触发
      triggerTransition();
    }
  }
};
...
/* 为 .fade-enter-active 和 .fade-leave-active 添加过渡规则 */
.fade-enter-active,
.fade-leave-active {
  transition: background-color 5s ease, opacity 5s ease, height 5s ease;
}
