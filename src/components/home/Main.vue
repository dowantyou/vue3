...
<template>
  ...
  <style scoped>
      ...
      .search-container textarea {
        ...
        max-height: 90px;
        overflow-y: hidden;
        word-wrap: break-word; /* 长单词或无法使用空白符分割的字符串，将被分割 */
      }
      ...
      .content {
        ...
        transform: translateY(100%); /* 初始状态，内容位于下方 */
        transition: opacity 0.5s ease, transform 0.5s ease;
      }
      ...
      .search-container textarea:focus ~ .content-wrapper .logo {
        opacity: 0;
        transform: scale(0.5); /* Logo缩小并向上移动 */
      }

      .search-container textarea:focus ~ .content-wrapper .content {
        opacity: 1;
        transform: translateY(0); /* 内容向上移动到视窗中心 */
      }
    </style>